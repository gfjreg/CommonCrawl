...
def Store_S3(fname,data):
    try:
        item = Key(BUCKET)
        item.key = JOB_QUEUE+'_'+fname.replace('/','_')[-30:]
...
def process_file(fileobj):
    try:
        return [line for line in fileobj if line[0] == '{' and 'facebook.com' in line] # returns all lines with JSON encoding which contains facebook.com
    except:
